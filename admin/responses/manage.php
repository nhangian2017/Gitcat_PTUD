...
if(isset($_GET['id']) && !empty($_GET['id'])){
	$qry = $conn->query("SELECT q.id, r.response_message, q.question, q.response_id FROM `questions` q inner join `responses` r on q.response_id = r.id where q.id = {$_GET['id']}");
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($_GET['id']) ? "Cập nhật ": "Thêm mới " ?>câu hỏi và câu trả lời</h3>
	</div>
	<div class="card-body">
		<form action="" id="response-form">
			...
			<div class="form-group">
				<label for="question_field" class="control-label">Câu hỏi</label>
                ...
                <small><i>(Nhấp vào "<b>+</b>" để thêm câu hỏi)</i></small>
			</div>
			<?php else: ?>
				<div class="form-group">
					<label for="question_field" class="control-label">Câu hỏi</label>
				...
				<div class="form-group">
					<label for="response_id_field" class="control-label">ID_Câu trả lời</label>
                    <input type="text" id="response_id_field" name="response_id[]" value="<?php echo isset($response_id) ? $response_id : '' ?>" class="form-control form-control-sm" required>
				</div>
			...
			<div class="form-group">
				<label for="response_message" class="control-label">Câu trả lời</label>
	...
	<div class="card-footer">
		<button class="btn btn-flat btn-primary" form="response-form">Lưu</button>
	...
	$(document).ready(function(){
		...
		$('#response-form').submit(function(e){
			...
			if($('input[name="question[]"]').length <= 0){
				...
				$('#question-holder').after("<span class='alert alert-danger err-msg'><i class='fa fa-exclamation-triangle'></i>  Bạn phải thêm ít nhất một câu hỏi để cho câu trả lời.</span>")
				$('#question').focus();
				return false;
			}
			start_loader();
