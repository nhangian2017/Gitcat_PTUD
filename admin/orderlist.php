...
$order = new order();
$processingOrderList = $order->getProcessingOrder(); //Đang processing order
$processedOrderList = $order->getProcessedOrder();// Đang processed order
$deliveringOrderList = $order->getDeliveringOrder();// Đang delivered order
$completeOrderList = $order->getCompleteOrder();// complete order
?>
                ...
                <table class="list">
                    ...
                    foreach ($processingOrderList as $key => $value) { ?>
                        <tr>
                            ...
                            <td><?= ($value['status'] != "Đang xử lý") ? $value['receivedDate'] : 
                            "Dự kiến 3 ngày sau khi đơn hàng đã được xử lý" ?> 
                            <?=  ($value['status'] != "Hoàn thành" && $value['status'] != "Đang xử lý") ? "(Dự kiến)" : "" ?> </td>
                            <td><?= $value['status'] ?></td>
                            <td>
