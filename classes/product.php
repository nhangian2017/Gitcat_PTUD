...
class product
{
    ...
    public function insert($data)
    {
        ...
        $query = "INSERT INTO products VALUES (NULL,'$name','$originalPrice','$promotionPrice','$unique_image',1,'" . date('Y/m/d') . "','$cateId','$qty','$des',1,0) ";
    ...
    //Hiển thị danh sách products trang admin
    public function getAllAdmin($page = 1, $total = 10)
    {
        ...
        $tmp = ($page - 1) * $total;
        $query ="SELECT * FROM products order by originalPrice asc limit $tmp,$total";
    ...
    public function getCountPagingClient($cateId, $row = 8)
    {
        ...
        if ($mysqli_result) {
            $totalrow = intval((mysqli_fetch_all($mysqli_result, MYSQLI_ASSOC)[0])['COUNT(*)']);
            $result = ceil($totalrow / $row); // Hàm làm tròn số
            return $result;
        }
        return false;
    }
    ///SELECT products lên trang chủ
    public function getFeaturedProducts()
    {
        $query =
            "SELECT *
			 FROM products
			 WHERE products.status = 1
             order by promotionPrice ASC    
             LIMIT 10";
        $result = $this->db->select($query);
        return $result;
    }
    public function getProductsByCateId($page = 1, $cateId, $total = 10)
    {
        if ($page <= 0) {
            $page = 1;
        }
    ...
    public function searchProduct($keyword)
        ...
        ///SELECT sản phẩm lên trang chủ ở mục Sách HOT
        public function getFeaturedProducts2()
        {
            $query =
                "SELECT *
                 FROM products
        ...
        public function getProductsByCateId2($page = 1, $cateId, $total = 10)
        {
            if ($page <= 0) {
                $page = 1;
            }
